...
def _campaign_draft_payload() -> dict[str, Any]:
    return {
        "schema_version": "campaign-draft-v1",
        "draft_id": "dashboard-simulation-draft",
        "business_goal": {
...
def _goal_candidate_payload() -> dict[str, Any]:
    return {
...
class DashboardApplication:
    ...
    def __init__(
        ...
        self._pending_goal_run_id: str | None = None
        ...
        self._pending_goal_run_id = self._recover_pending_goal_run_id()
    ...
    def run_goal_simulation(self, semantic_decision: str) -> dict[str, Any]:
        ...
        result = self.workflows.run_goal(
            run_id=run_id,
            proposal_id="proposal-" + run_id,
            candidate_payload=_goal_candidate_payload(),
    ...
    def run_goal_technical_simulation(self) -> dict[str, Any]:
        if self._pending_goal_run_id is not None:
        ...
        self._pending_goal_run_id = run_id
        return result

    def _recover_pending_goal_run_id(self) -> str | None:
        candidates = sorted(
            (
                path
                for path in self.runs_root.iterdir()
                if path.is_dir()
                and (path / "goal_technical.json").is_file()
                and not (path / "goal_workflow.json").exists()
            ),
    ...
    def decide_pending_goal_simulation(
        ...
        if self._pending_goal_run_id is None:
            raise ValueError("GOAL_TECHNICAL_VERIFICATION_REQUIRED")
        run_id = self._pending_goal_run_id
        ...
        self._pending_goal_run_id = None
        self._write_workflow_evidence(result, "GOAL_LIFECYCLE")
        return result

    def run_impact_fixture(self, fixture_name: str) -> dict[str, Any]:
