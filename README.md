...
The separate production P0 candidate is available at `http://127.0.0.1:8878/strategy`. Following the project-wide AI-first rule, the agent owns all safe work and delegates only critical decisions or material uncertainty to the operator. It researches up to six relevant first-party public HTTPS pages, combines that evidence with the connected Direct campaign catalog and Metrika performance context, prepares a sourced business model and Campaign Strategy, and durably revises the exact one-campaign publish projection. It contains no Test Scenario fallback. The final creation control is absent until the tracked policy binds the current Direct account and single writer, production write is explicitly authorized, and `MOX_ADV_DIRECT_PILOT_WRITE` exists in macOS Keychain. When armed, the adapter uses only official Direct v501 methods and must confirm `State=SUSPENDED`; P0 never calls `Campaigns.resume`.
